...
const { sendMessage, getThread, listConversations, markMessageRead } = require('../controllers/messageController');

router.post('/', protect, tenantScope, authorize('parent', 'assistant', 'admin'), sendMessage);
router.get('/conversations', protect, tenantScope, authorize('parent', 'assistant', 'admin'), listConversations);
